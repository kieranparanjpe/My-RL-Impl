...
class Trainer:
    ...
    def __init__(self, environment_id, algorithm_id : str, policy_id : str, hyperparameters : Hyperparameters, logging=True):
    ...
    def train(self):
        ...
        for timestep in tqdm(range(self.hyperparameters.n_timesteps)):
            ...
            self.algorithm.update_and_observe(last_observation, next_observation, action, log_prob_action, reward,
                                              done, timestep)
...
if __name__ == "__main__":
    trainer = Trainer("CartPole-v1", "ppo", "categorical", PPOHyperparams(), logging=True)
    trainer.train()
